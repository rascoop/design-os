---
description: Build props-based section screen components
agent: build
---
...
If there's only one section, auto-select it. If there are multiple sections, use the `question` tool to ask which section the user wants to create a screen design for.
...
- Check if `/product/design-system/style-notes.md` exists (optional)
...
Before implementation, use the `skill` tool to load `app-design-patterns`, then present 2-3 UI options for the chosen view and ask the user to pick one. Example options:

- **Option A: Dense Utility** — compact table/list, high information density, faster scanning
- **Option B: Balanced Workspace** — cards + clear hierarchy, moderate spacing, flexible actions
- **Option C: Visual First** — larger cards/panels, stronger emphasis on status and narrative

For each option, include 1-2 lines on tradeoffs (speed to scan, complexity, mobile behavior).

If the spec implies multiple views, use the `question` tool to confirm which view to build first:
...
If there's only one obvious view, still present 2-3 UI options for that view and get a selection before coding.

## Step 5: Invoke Design Skills

Before creating the screen design, use the `skill` tool to load both:

- `frontend-design` for visual quality and styling craft
- `app-design-patterns` for structural pattern quality and option tradeoffs
...
**If `/product/design-system/style-notes.md` exists:**
- Apply density to spacing, row heights, and content grouping
- Apply radius preference to cards/buttons/inputs
- Apply motion preference to transitions/animations
- Apply contrast/chrome preference to borders and shadows
...
- Include explicit empty, loading, and error states whenever relevant to the section
...
## Step 10: Optional UI Quality Review

After generating files, optionally load `web-design-guidelines` and run a quick compliance review on the new screen files.

Focus on:

- accessibility basics (focus states, labels, contrast)
- responsive behavior and overflow
- interaction affordances and empty/error state clarity

Apply obvious fixes immediately before final confirmation.
...
- ALWAYS load `frontend-design` and `app-design-patterns` with the `skill` tool before creating screen designs
- Use `web-design-guidelines` for a final quality pass when available
