---
description: Choose product colors, typography, and UI style defaults
agent: build
---
...
Before proposing token options, load `tailwind-design-system` and `design-system-patterns` with the `skill` tool when available.

"Let's define the visual identity for **[Product Name]**.

I'll help you choose:
1. **Visual style pack** — overall UI direction and interaction feel
2. **Colors** — a primary accent, secondary accent, and neutral palette
3. **Typography** — fonts for headings, body text, and code
...
Before colors and fonts, present style-pack options and ask the user to choose one:

- **Operational Dashboard** — compact density, medium radius, high information density
- **Editorial Product** — roomy spacing, elegant typography contrast, restrained accents
- **Friendly Consumer** — soft radius, larger touch targets, approachable tone
- **Enterprise Control Plane** — neutral-forward palette, strong hierarchy, clear status semantics
- **Custom Mix** — user picks each setting manually

Then capture these UI options (using the `question` tool):

- Density: `compact` | `comfortable` | `spacious`
- Corner radius: `sharp` | `soft` | `rounded`
- Motion level: `minimal` | `moderate` | `expressive`
- Contrast preference: `standard` | `high`
- Component chrome: `subtle` | `defined` (borders/shadows prominence)
...
Use the `question` tool to gather their preferences if they're unsure:
...
**File 3:** `/product/design-system/style-notes.md`
```markdown
# Style Notes
...
- `/product/design-system/style-notes.md`
...
- Save UI style decisions in `style-notes.md` so `/design-shell` and `/design-screen` can apply them consistently
