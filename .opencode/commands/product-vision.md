---
description: Define product vision, roadmap, and data shape
agent: build
---
...
Before questioning, use the `skill` tool to load `prd-discovery` and apply its discovery framework.

After receiving their input, use the `question` tool to ask targeted questions covering all three areas. Ask questions one or two at a time, conversationally, with follow-ups as needed.

Minimum coverage checklist before writing files:

- Primary user segment(s), role, and context of use
- Current alternatives/workarounds and why they fail
- Top pain points ranked by severity and frequency
- Success outcomes (user success + business success)
- MVP scope vs later-phase ideas (explicit non-goals)
- Constraints (timeline, platform, compliance/privacy concerns)

Do not proceed to file generation until each checklist item has a concrete answer or an explicit assumption.
...
- "What does success look like 30 days after adoption?"
...
- "Which section is highest risk or most uncertain and needs extra design validation?"
...
- "What entity state or lifecycle matters most for the UI (draft, active, archived, etc.)?"
